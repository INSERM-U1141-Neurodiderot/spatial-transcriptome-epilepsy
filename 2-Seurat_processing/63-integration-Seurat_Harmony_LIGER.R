...
sct_variables_to_regress <- "condition"
scale_data_variables_to_regress <- NA
variables_to_regress_in_dir <- "condition"
scale_data_split_by <- "condition"
scale_data_split_by_in_dir <- "condition"
integration_group <- "condition"
integration_group_by_in_dir <- "condition"
...
#clustering_resolution_vector <- c(0.5, 0.8)
#features_param <- 500
...
qc_output_dir <- sprintf("%s/Merge_norm_%s_scale_%s-int_%s-QC-sum%d_det%s_mito%d_hb%d", output_dir, variables_to_regress_in_dir, scale_data_split_by_in_dir, integration_group_by_in_dir, spot_sum_of_counts_threshold, spot_detected_genes_threshold, spot_mito_gene_count_pct_threshold, spot_hb_gene_count_pct_threshold)
...
SpatialExperiment_Seurat_conversion <- function(sp_exp, sr_dir, add_image) {
    ...
    metadata_df <- merge(as.data.frame(colData(sp_exp))[, c("barcode", "in_tissue", "array_row", "array_col", "lame", "zone", "condition", colnames(colData(sp_exp))[grepl("_percent", colnames(colData(sp_exp)))])], as.data.frame(sp_exp@int_colData$spatialCoords), by.x="barcode", by.y="row.names")
...
seurat_feature_stat_plots <- function(seurat_obj, feature_name, suffix, plot_legend_title) {
    nb_images <- length(names(seurat_obj@images))
    plots2return <- list()
...
seurat_all_feature_stat_plots <- function(seurat_obj, assay2use, col_prefix, plot_title, out_dir, out_name) {
    sample <- seurat_obj@project.name
    nb_images <- length(names(seurat_obj@images))
    pdf(sprintf("%s/%s_%s_Seurat_features.pdf", out_dir, sample, out_name))
    # counts per spot
    plot_legend_title <- "count sum"
    raw_plots <- seurat_feature_stat_plots(seurat_obj, "nCount", assay2use, plot_legend_title)
    if (nb_images == 1) {
        print((wrap_plots(raw_plots[["violin"]], raw_plots[["spatial"]])) + plot_annotation(title=plot_title))
    } else {
        print(raw_plots[["violin"]] + plot_annotation(title=plot_title))
        print(raw_plots[["spatial"]] + plot_annotation(title=plot_title))
    }
    # number of expressed genes per spot
    plot_legend_title <- "detected genes"
    raw_plots <- seurat_feature_stat_plots(seurat_obj, "nFeature", assay2use, plot_legend_title)
    if (nb_images == 1) {
        print((wrap_plots(raw_plots[["violin"]], raw_plots[["spatial"]])) + plot_annotation(title=plot_title))
    } else {
        print(raw_plots[["violin"]] + plot_annotation(title=plot_title))
    ...
    raw_plots <- seurat_feature_stat_plots(seurat_obj, sprintf("%s_mito_percent", col_prefix), assay2use, plot_legend_title)
    if (nb_images == 1) {
        print((wrap_plots(raw_plots[["violin"]], raw_plots[["spatial"]])) + plot_annotation(title=plot_title))
    } else {
        print(raw_plots[["violin"]] + plot_annotation(title=plot_title))
    ...
    raw_plots <- seurat_feature_stat_plots(seurat_obj, sprintf("%s_hb_percent", col_prefix), assay2use, plot_legend_title)
    if (nb_images == 1) {
        print((wrap_plots(raw_plots[["violin"]], raw_plots[["spatial"]])) + plot_annotation(title=plot_title))
    } else {
        print(raw_plots[["violin"]] + plot_annotation(title=plot_title))
...
seurat_merge_normalization <- function(seurat_merge, method, vars2regress, feat_param, only_var, mito_genes, hb_genes, out_dir) {
    if (method == "SCTransform") {
        if (feat_param > 10) {
            ### feat_param is a number of features
            print(sprintf("features param: %d", feat_param))
            if (! is.na(vars2regress)) {
                vars2regress_in_dirname <- paste(gsub("[.]", "", vars2regress), collapse="-")
                seurat_merge_norm <- SCTransform(seurat_merge, variable.features.n=feat_param, return.only.var.genes=only_var, vars.to.regress=vars2regress)
            } else {
                vars2regress_in_dirname <- "no_vars_to_regress"
                seurat_merge_norm <- SCTransform(seurat_merge, variable.features.n=feat_param, return.only.var.genes=only_var)
            }
            normalization_out_dir <- sprintf("%s/00-SCTransform/%s/featnb%d", out_dir, vars2regress_in_dirname, feat_param)
            ...
            if (! is.na(vars2regress)) {
                vars2regress_in_dirname <- paste(gsub("[.]", "", vars2regress), collapse="-")
                seurat_merge_norm <- SCTransform(seurat_merge, variable.features.n=NULL, variable.features.rv.th=feat_param, return.only.var.genes=only_var, vars.to.regress=vars2regress)
            } else {
                vars2regress_in_dirname <- "no_vars_to_regress"
                seurat_merge_norm <- SCTransform(seurat_merge, variable.features.n=NULL, variable.features.rv.th=feat_param, return.only.var.genes=only_var)
            }
            normalization_out_dir <- sprintf("%s/00-SCTransform/%s/resvar%s", out_dir, vars2regress_in_dirname, sub("[.]", "_", feat_param))
        ...
        selection_method <- "sct"
        ## add feature statistics to Seurat object after SCTransform
        seurat_merge_norm <- addFeatureStats2Seurat(seurat_merge_norm, default_assay)
    } else {
        if (method == "LogNormalize") {
            selection_method <- "vst"
            ### feat_param is a number of features
    ...
    seurat_all_feature_stat_plots(seurat_merge_norm, default_assay, norm_column_prefix, "Normalized counts", normalization_out_dir, "normalized_QC_stats") 
    detach("package:RColorBrewer")
    #### mitochondrial and hemoglobin gene spatial plots after filerting
    ##### hemoglobin genes
    df_list <- qc_spatial_mean_exp_plots(seurat_merge_norm, hb_genes, TRUE, normalization_out_dir, "normalized_hemoglobin_genes")
    ##### mitochondrial genes
    df_list <- qc_spatial_mean_exp_plots(seurat_merge_norm, mito_genes, TRUE, normalization_out_dir, "normalized_mitochondrial_genes")
...
library(RColorBrewer)
lapply(seurat_objects, seurat_all_feature_stat_plots, assay2use="RNA", col_prefix=qc_column_prefix, plot_title="Raw counts after QC", out_dir=qc_stats_output_dir, out_name="filtered_QC_stats") 
detach("package:RColorBrewer")
#### mitochondrial and hemoglobin gene spatial plots after filerting
##### hemoglobin genes
...
seurat_objects_merge <- addFeatureStats2Seurat(seurat_objects_merge, "RNA")
...
for (normalization_method in normalization_method_vector) {
    ...
    for (features_param in features_param_vector) {
        ...
        returned_list <- seurat_merge_normalization(seurat_objects_merge, normalization_method, sct_variables_to_regress, features_param, return_only_var_genes, mitochondrial_geneset_spe, hemoglobin_geneset_spe, integration_output_dir)
        seurat_objects_merge_norm <- returned_list[["seurat_merge"]]
        ...
        ## HVG plots
        hvg_df <- hvg_plots(seurat_objects_merge_norm, default_assay, selection_method, features_param, normalization_output_dir, "HVGs")
        
        if (normalization_method == "LogNormalize") {
            if (! is.na(scale_data_split_by)) {
                if (!is.na(scale_data_variables_to_regress)) {
                    seurat_objects_merge_norm <- ScaleData(seurat_objects_merge_norm, split.by=scale_data_split_by, vars.to.regress=scale_data_variables_to_regress)
                    normalization_output_dir <- sprintf("%s/%s_%s", normalization_output_dir, gsub("[.]", "", scale_data_split_by), paste(gsub("[.]", "", scale_data_variables_to_regress), collapse="-"))
                } else {
                    seurat_objects_merge_norm <- ScaleData(seurat_objects_merge_norm, split.by=scale_data_split_by)
                    normalization_output_dir <- sprintf("%s/%s", normalization_output_dir, gsub("[.]", "", scale_data_split_by))
                }
            } else {
                seurat_objects_merge_norm <- ScaleData(seurat_objects_merge_norm)
            }
        }
        
        ## dimensionality reduction
        seurat_objects_merge_norm <- RunPCA(seurat_objects_merge_norm, verbose=FALSE)
        reduction2use <- "pca"
        for (dimensions_nb_param in dimensions_nb_param_vector) {
            print(sprintf("dimension number: %d", dimensions_nb_param))
            dimensions_output_dir <- sprintf("%s/%d_dims", normalization_output_dir, dimensions_nb_param)
            if (! dir.exists(dimensions_output_dir)) {
                dir.create(dimensions_output_dir, recursive=TRUE, mode="0775")
            }
            dimensions_output_name <- sprintf("%s_noIntegration_%ddims", normalization_output_name, dimensions_nb_param)

            seurat_objects_merge_norm <- FindNeighbors(seurat_objects_merge_norm, reduction=reduction2use, dims=1:dimensions_nb_param)
            ...
            for (metadata_category_colname in metadata_categories) {
                seurat_objects_merge_norm <- compute_silhouette(seurat_objects_merge_norm, reduction2use, dimensions_nb_param, metadata_category_colname)
                mean_silhouette <- mean(seurat_objects_merge_norm@meta.data[[sprintf("silhouette_%s", metadata_category_colname)]])
            ...
            for (clustering_resolution in clustering_resolution_vector) {
                ...
                seurat_objects_merge_norm <- clustering(seurat_objects_merge_norm, clustering_resolution, reduction2use, dimensions_nb_param)
                write.csv(seurat_objects_merge_norm@meta.data, file=sprintf("%s/%s_metadata.csv", resolution_output_dir, resolution_output_name), quote=FALSE, row.names=TRUE)
                cluster_mean_silhouette <- mean(seurat_objects_merge_norm$silhouette_seurat_clusters)
                ### plot clusters onto UMAP or onto the tissue section
                clustering_plots(seurat_objects_merge_norm, load_image, nb_SpatialFeaturePlot_per_page, "Non-integrated datasets, all spots", resolution_output_dir, sprintf("%s_clustering", resolution_output_name))

                ### hippocampus spots
                seurat_objects_merge_norm_hippocampus <- subset(seurat_objects_merge_norm, cells=all_sections_hippocampus_barcodes_Seurat_merge)
                hippocampus_cluster_mean_silhouette <- mean(seurat_objects_merge_norm_hippocampus$silhouette_seurat_clusters)
                #### plot clusters onto UMAP or onto the tissue section
                clustering_plots(seurat_objects_merge_norm_hippocampus, load_image, nb_SpatialFeaturePlot_per_page, "Non-integrated datasets, hippocampus spots", resolution_output_dir, sprintf("%s_clustering_hippocampus", resolution_output_name))
                ...
                rm(seurat_objects_merge_norm_hippocampus)
                gc()
            }
        }
        rm(seurat_objects_merge_norm)
        gc()
    }
}
...
for (normalization_method in normalization_method_vector) {
    ...
    for (features_param in features_param_vector) {
        ...
        seurat_object_list <- SplitObject(seurat_objects_merge_norm, split.by="orig.ident")
        rm(seurat_objects_merge_norm)
        gc()
        ### after splitting, all the objects still have all the images: only keep the image of the corresponding sample
        for (one_sample in names(seurat_object_list)) {
            seurat_object_list[[one_sample]]@images <- list(seurat_object_list[[one_sample]]@images[[one_sample]])
            names(seurat_object_list[[one_sample]]@images) <- one_sample
        }
        
        ## integration
        integration_output_name <- sprintf("%s_%s", normalization_output_name, integration_method)
        features <- SelectIntegrationFeatures(seurat_object_list, nfeatures=integration_features_nb)
        ...
        int.anchors <- FindIntegrationAnchors(object.list=seurat_object_list, normalization.method=integration_normalization_method, anchor.features=features)
        ...
        if (normalization_method == "LogNormalize") {
            if (! is.na(scale_data_split_by)) {
                if (!is.na(scale_data_variables_to_regress)) {
                    seurat_objects_integrated <- ScaleData(seurat_objects_integrated, split.by=scale_data_split_by, vars.to.regress=scale_data_variables_to_regress)
                    normalization_output_dir <- sprintf("%s/%s_%s", normalization_output_dir, gsub("[.]", "", scale_data_split_by), paste(gsub("[.]", "", scale_data_variables_to_regress), collapse="-"))
                } else {
                    seurat_objects_integrated <- ScaleData(seurat_objects_integrated, split.by=scale_data_split_by)
                    normalization_output_dir <- sprintf("%s/%s", normalization_output_dir, gsub("[.]", "", scale_data_split_by))
                }
            } else {
        ...
        for (dimensions_nb_param in dimensions_nb_param_vector) {
            ...
            dimensions_output_name <- sprintf("%s_%ddims", integration_output_name, dimensions_nb_param)
...
for (normalization_method in normalization_method_vector) {
    ...
    for (features_param in features_param_vector) {
        ...
        for (dimensions_nb_param in dimensions_nb_param_vector) {
            ...
            dimensions_output_name <- sprintf("%s_%s_%ddims", normalization_output_name, integration_method, dimensions_nb_param)

            ### integration and dimensionality reduction
            seurat_objects_integrated <- RunHarmony(seurat_objects_merge_norm, group.by.vars=integration_group, dims=1:dimensions_nb_param)
...
for (normalization_method in normalization_method_vector) {
    ...
    for (features_param in features_param_vector) {
        ...
        if (normalization_method == "LogNormalize") {
            if (! is.na(scale_data_split_by)) {
                if (!is.na(scale_data_variables_to_regress)) {
                    seurat_objects_merge_norm <- ScaleData(seurat_objects_merge_norm, split.by=scale_data_split_by, vars.to.regress=scale_data_variables_to_regress, do.center=FALSE)
                    normalization_output_dir <- sprintf("%s/%s_%s", normalization_output_dir, gsub("[.]", "", scale_data_split_by), paste(gsub("[.]", "", scale_data_variables_to_regress), collapse="-"))
                } else {
                    seurat_objects_merge_norm <- ScaleData(seurat_objects_merge_norm, split.by=scale_data_split_by, do.center=FALSE)
                    normalization_output_dir <- sprintf("%s/%s", normalization_output_dir, gsub("[.]", "", scale_data_split_by))
                }
            } else {
                seurat_objects_merge_norm <- ScaleData(seurat_objects_merge_norm, do.center=FALSE)
            }
        }
        ...
        for (dimensions_nb_param in dimensions_nb_param_vector) {
            ...
            ### integration and dimensionality reduction
            seurat_objects_integrated <- RunOptimizeALS(seurat_objects_merge_norm, k=dimensions_nb_param, lambda=5, split.by=integration_group)
            seurat_objects_integrated <- RunQuantileNorm(seurat_objects_integrated, split.by=integration_group)
            reduction2use <- "iNMF"
...
gc()
write.csv(integration_cluster_mean_silhouette_df, file=sprintf("%s/featuresnumber_integration_dims_resolution_cluster_mean_silhouette.csv", qc_filtering_output_dir), quote=FALSE, row.names=FALSE)
...
pdf(sprintf("%s/featuresnumber_integration_dims_resolution_cluster_mean_silhouette.pdf", qc_filtering_output_dir))
...
write.csv(integration_batch_mean_silhouette_df, file=sprintf("%s/featuresnumber_integration_dims_resolution_batch_mean_silhouette.csv", qc_filtering_output_dir), quote=FALSE, row.names=FALSE)
...
pdf(sprintf("%s/featuresnumber_integration_dims_resolution_batch_mean_silhouette.pdf", qc_filtering_output_dir))
