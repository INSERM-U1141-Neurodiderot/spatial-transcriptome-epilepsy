...
for (category_colname in c("seurat_clusters", "condition", "time", "orig.ident")) {
    p <- DimPlot(seurat_object_whole_clustering, reduction="umap", group.by=category_colname, label=TRUE)
...
#umap_plot_clusters <- DimPlot(seurat_object_whole_clustering, reduction="umap", group.by="seurat_clusters", label=TRUE)
#umap_plot_clusters <- umap_plot_clusters + labs(title="") + theme(legend.position="none") + theme(axis.title.x=element_text(size=7), axis.title.y=element_text(size=7), axis.text.x=element_text(size=6), axis.text.y=element_text(size=6))
#umap_plot_condition <- DimPlot(seurat_object_whole_clustering, reduction="umap", group.by="condition", label=TRUE)
#umap_plot_condition <- umap_plot_condition + labs(title="") + theme(legend.position="top") + theme(axis.title.x=element_text(size=7), axis.title.y=element_text(size=7), axis.text.x=element_text(size=6), axis.text.y=element_text(size=6), legend.text=element_text(size=6))
#umap_plot_time <- DimPlot(seurat_object_whole_clustering, reduction="umap", group.by="time", label=TRUE)
#umap_plot_time <- umap_plot_time + labs(title="") + theme(legend.position="top") + theme(axis.title.x=element_text(size=7), axis.title.y=element_text(size=7), axis.text.x=element_text(size=6), axis.text.y=element_text(size=6), legend.text=element_text(size=6))
#umap_plot_sample <- DimPlot(seurat_object_whole_clustering, reduction="umap", group.by="orig.ident", label=TRUE)
#umap_plot_sample <- umap_plot_sample + labs(title="") + theme(legend.position="top") + theme(axis.title.x=element_text(size=7), axis.title.y=element_text(size=7), axis.text.x=element_text(size=6), axis.text.y=element_text(size=6), legend.text=element_text(size=5))

## combine spatial plot and legend
#umap_plot_sample_legend <- get_legend(umap_plot_sample)
#umap_plot_sample <- umap_plot_sample + theme(legend.position="none") 
#umap_plot_sample_figure <- ggdraw() + 
#    draw_plot(umap_plot_sample, 0, 0, 1, 0.9) +
#    draw_plot(umap_plot_sample_legend, 0.3, 0.85, 0.4, 0.1)
#print(umap_plot_sample_figure)


#umap_figure <- (umap_plot_clusters + umap_plot_condition) / (umap_plot_time + umap_plot_sample)
#print(umap_figure)
...
marker_figure_plot <- function(seurat_obj, gene, palette) {
    ...
    violin_plot <- violin_plot + labs(x="Cluster") + theme(legend.position="none")
...
one_gene <- "Tbr1"
#### set color palette based on gene max expression
one_gene_max_exp <- max(seurat_object_whole_clustering_subset@assays$SCT@data[one_gene,])
sc <- scale_fill_gradientn(colours=myPalette(100), limits=c(0, one_gene_max_exp))
#### violin plot
violin_plot <- VlnPlot(object=seurat_object_whole_clustering_subset, features=one_gene)
violin_plot <- violin_plot + labs(x="Cluster") + theme(legend.position="none")
#### expression on tissue section
spatial_plot <- SpatialFeaturePlot(object=seurat_object_whole_clustering_subset, features=one_gene, alpha=c(0.1,1), ncol=2)
spatial_plot <- spatial_plot & sc
##### get legend
spatial_plot_legend <- get_legend(spatial_plot)
##### spatial plot without legend
spatial_plot <- spatial_plot & labs(title="") & theme(legend.position="none")
##### combine spatial plot and legend
spatial_plot_figure <- ggdraw() +
    draw_plot(spatial_plot, 0, 0.15, 1, 0.85) +
    draw_plot(spatial_plot_legend, 0, 0, 1, 0.15)
## combine violin plot, spatial plot and legend
#marker_table_df <- data.frame(gene=c("test", "test1", "test2"), cluster=c("0", "1", "2"), ref=c("1", "2", "3"))
#marker_figure <- (violin_plot / spatial_plot_figure + plot_layout(heights=c(1,3))) | gridExtra::tableGrob(marker_table_df) 
#print(marker_figure)
marker_figure <- violin_plot / spatial_plot_figure + plot_layout(heights=c(1,3))
...
#figure <- ggdraw() +
#    draw_plot(clustering_figure, 0, 0.5, 1, 0.5) +
#    draw_plot(marker_figure, 0, 0, 1, 0.5)
#pdf(sprintf("%s/whole_clustering_figure.pdf", fig_output_dir), width=9)
#print(figure)
#dev.off()

#plot_grid(umap_figure, clustering_splatial_plot, labels = c('A', 'B'), label_size = 12)
...
#for (one_cluster in c("0", "3")) {
    one_cluster_fig_output_dir <- sprintf("%s/c%s", one_fig_output_dir, one_cluster)
    ## UMAP plots
    dimplot_list <- list()
    for (category_colname in c("seurat_clusters", "condition", "time", "orig.ident")) {
        p <- DimPlot(seurat_object_subclustering_list[[one_cluster]], reduction="umap", group.by=category_colname, label=TRUE)
        if (length(levels(seurat_object_subclustering_list[[one_cluster]]@meta.data[[category_colname]])) > 1) {
            p <- p + ggtitle(sprintf("Mean silhouette: %.2f", mean(seurat_object_subclustering_list[[one_cluster]]@meta.data[[sprintf("silhouette_%s", category_colname)]]))) + theme(plot.title=element_text(size=12)) + theme(legend.position="bottom")
        }
        dimplot_list[[category_colname]] <- p
    ...
    ## clustering on tissue sections
    one_timepoint_metadata_df <- seurat_object_subclustering_list[[one_cluster]]@meta.data[which(seurat_object_subclustering_list[[one_cluster]]@meta.data$time==one_timepoint),]
    one_timepoint_metadata_df <- droplevels(one_timepoint_metadata_df)
    sections <- levels(one_timepoint_metadata_df$orig.ident)
    ### plot to get legend
    clustering_splatial_plot <- SpatialDimPlot(seurat_object_subclustering_list[[one_cluster]], images=sections[1], label=TRUE, label.size=2)
    clustering_splatial_plot <- clustering_splatial_plot + labs(fill="cluster")
    clustering_splatial_plot_legend <- get_legend(clustering_splatial_plot)
    ### plot with all timepoint sections, without legend
    clustering_splatial_plot <- SpatialDimPlot(seurat_object_subclustering_list[[one_cluster]], images=sections, ncol=2, label=TRUE, label.size=2) & theme(plot.title=element_text(size=10), legend.position="none")
    ### combine spatial plot and legend
    clustering_splatial_plot_figure <- ggdraw() +
        draw_plot(clustering_splatial_plot, 0, 0, 0.9, 1) +
        draw_plot(clustering_splatial_plot_legend, 0.9, 0, 0.1, 1)
    ...
    for (one_marker in one_cluster_markers) {
        marker_figure <- marker_figure_plot(seurat_object_whole_clustering_subset, one_marker, myPalette)
