...
# normalization
spe_seurat <- SCTransform(spe_seurat, assay = "RNA")
# counts per spot
...
spe_seurat_norm_comp <- SCTransform(spe_seurat, assay = "RNA", return.only.var.genes=FALSE, verbose=FALSE)
...
clustering_output_dir <- sprintf("%s/20-Clustering", output_dir)
if (! dir.exists(clustering_output_dir)) {
    dir.create(clustering_output_dir, recursive=TRUE, mode="0775")
}
...
nb_pcs <- round(dim_estimate$dim.est)
df2ggplot <- data.frame(PC=1:total_pcs, stdev=spe_seurat@reductions$pca@stdev)
write.csv(df2ggplot, file=sprintf("%s/PCs_stdev.csv", clustering_output_dir), quote=FALSE, row.names=TRUE)
pdf(sprintf("%s/PCs_stdev.pdf", clustering_output_dir))
...
nb_dimensions_list <- list()
#for (dim2use in nb_pcs, seurat_spatial_vignette_pcs)) {
for (dim2use in nb_pcs:total_pcs) {
    print(sprintf("nb PCs: %d", dim2use))
    clustering_output_sudir <- sprintf("%s/%s_PCs", clustering_output_dir, dim2use)
    if (! dir.exists(clustering_output_sudir)) {
        dir.create(clustering_output_sudir, recursive=TRUE, mode="0775")
    }

    spe_seurat <- FindNeighbors(spe_seurat, reduction="pca", dims=1:dim2use)
    spe_seurat <- FindClusters(spe_seurat, verbose=FALSE)
    spe_seurat <- RunUMAP(spe_seurat, reduction="pca", dims=1:dim2use)
    ...
    dev.off()
    one_nb_dimensions_list <- list()
    one_nb_dimensions_list[["spatilal_dimplot"]] <-  SCT_cluster_plot2
    SCT_cluster_plot2_no_legend <- SCT_cluster_plot2 + NoLegend()
    one_nb_dimensions_list[["spatilal_dimplot_no_legend"]] <-  SCT_cluster_plot2_no_legend
    ...
    one_nb_dimensions_list[["mean_silhouette_score"]] <- mean_silhouette_score
    ...
    nb_dimensions_list[[sprintf("%d_PCs", dim2use)]] <- one_nb_dimensions_list
...
print(nb_dimensions_list[["15_PCs"]][["spatilal_dimplot_no_legend"]] + NoLegend() + nb_dimensions_list[["30_PCs"]][["spatilal_dimplot_no_legend"]] + NoLegend() + plot_annotation(title="Number of PCs comparison"))
print(nb_dimensions_list[["15_PCs"]][["spatilal_dimplot_no_legend"]] + plot_annotation(title=sprintf("%d PCs: %f", 15, nb_dimensions_list[["15_PCs"]][["mean_silhouette_score"]])))
...
for (dim2use in nb_pcs:total_pcs) {
    # add title 
    #spatial_dimplot[[sprintf("%d_PCs", dim2use)]] <- nb_dimensions_list[[sprintf("%d_PCs", dim2use)]][["spatilal_dimplot_no_legend"]] + plot_annotation(title=sprintf("%d PCs: %f", dim2use, nb_dimensions_list[[sprintf("%d_PCs", dim2use)]][["mean_silhouette_score"]]))
    spatial_dimplot[[sprintf("%d_PCs", dim2use)]] <- nb_dimensions_list[[sprintf("%d_PCs", dim2use)]][["spatilal_dimplot_no_legend"]] + labs(title=sprintf("%d PCs: %f", dim2use, nb_dimensions_list[[sprintf("%d_PCs", dim2use)]][["mean_silhouette_score"]]))
    #print(spatial_dimplot[[sprintf("%d_PCs", dim2use)]])
    mean_silhouette_score_vector <- c(mean_silhouette_score_vector, nb_dimensions_list[[sprintf("%d_PCs", dim2use)]][["mean_silhouette_score"]])
}
print(wrap_plots(spatial_dimplot) + plot_annotation(title="Number of PCs comparison"))

df2ggplot <- data.frame(pcs=nb_pcs:total_pcs, silhouette=mean_silhouette_score_vector)
write.csv(df2ggplot, file=sprintf("%s/PCs_mean_silhouette_score.csv", clustering_output_dir), quote=FALSE, row.names=TRUE)
pdf(sprintf("%s/PCs_mean_silhouette_score.pdf", clustering_output_dir))
p <- ggplot(df2ggplot, aes(x=pcs, y=silhouette)) +
    geom_point() +
    geom_line() +
