...
custom_cluster_comp_output_dir <- sprintf("%s/%s/%s/custom/10-DEGs/%s/%s/dynamics2", sub_markers_output_dir, qc_filtering_output_subdir, subclustering_integration, markers_test, markers_latent_variables_in_path)
...
all_times_dynamics_go_terms[["2"]] <- c("axonogenesis", "axon development", "vesicle-mediated transport in synapse", "synaptic vesicle cycle", "glutamate receptor signaling pathway", "ionotropic glutamate receptor signaling pathway", "calcium-mediated signaling", "second-messenger-mediated signaling", "synapse maturation", "regulation of neurotransmitter receptor activity", "potassium ion transmembrane transport")
all_times_dynamics_go_terms[["3"]] <- c("neural nucleus development", "modulation of chemical synaptic transmission", "regulation of membrane potential", "synapse organization", "regulation of trans-synaptic signaling", "regulation of transmembrane transport")
all_times_dynamics_go_terms[["4"]] <- c("synapse pruning")
all_times_dynamics_go_terms[["5"]] <- c("oxidative phosphorylation", "generation of precursor metabolites and energy", "NADH dehydrogenase complex assembly", "mitochondrial respiratory chain complex I assembly", "proton transmembrane transport", "mitochondrion organization", "mitochondrial respiratory chain complex assembly", "electron transport chain")
...
### 5 dynamics: immune respons (regulation of immune response), axon/synaptic transmission (axonogenesis), synaptic transmission (modulation of chemical synaptic transmission), synapse pruning, cellular metabolism (mitochondrial respiratory chain complex I assembly)
...
spe_sub_go_pval_2 <- spe2speGOpval(spe_sub, "GO:0007409", all_times_GO_ORA_results2compare_df, "2")
...
spe_sub_go_pval_3 <- spe2speGOpval(spe_sub, "GO:0050804", all_times_GO_ORA_results2compare_df, "3")
sample_ids <- c(sample_ids, spe_sub_go_pval_3@int_metadata$imgData$sample_id)
sample_id_times <- c(sample_id_times, rep("", 4)) # only plot times for the first GO ID
spe_sub_go_pval_4 <- spe2speGOpval(spe_sub, "GO:0098883", all_times_GO_ORA_results2compare_df, "4")
sample_ids <- c(sample_ids, spe_sub_go_pval_4@int_metadata$imgData$sample_id)
sample_id_times <- c(sample_id_times, rep("", 4)) # only plot times for the first GO ID
spe_sub_go_pval_5 <- spe2speGOpval(spe_sub, "GO:0032981", all_times_GO_ORA_results2compare_df, "5")
...
# other GO term sets
other_GO_output_dir <- sprintf("%s/%s/%s/custom/10-DEGs/%s/%s/sets2", sub_markers_output_dir, qc_filtering_output_subdir, subclustering_integration, markers_test, markers_latent_variables_in_path)
...
all_times_other_go_terms[["axon"]] <- c(grep("axon", unique(all_times_GO_ORA_results2compare_df$Description), value=TRUE), grep("dendrit", unique(all_times_GO_ORA_results2compare_df$Description), value=TRUE))
all_times_other_go_terms[["myelin"]]  <- c(grep("myelin", unique(all_times_GO_ORA_results2compare_df$Description), value=TRUE), grep("ensheathment", unique(all_times_GO_ORA_results2compare_df$Description), value=TRUE))
...
all_times_other_go_terms[["translation"]]  <- c("ribosome assembly", "ribosomal small subunit assembly", "ribosome biogenesis", "ribosomal small subunit biogenesis", "translational elongation", "positive regulation of translation")
all_times_other_go_terms[["glucose"]] <- c("NADH regeneration", "canonical glycolysis", "glucose catabolic process to pyruvate")
