...
hippocampus_cluster.v <- c("3.0","3.1","3.2","3.3")
cerebral_cluster.v <- c("0.0","0.1","0.2","0.3")
...
top_markers.v <- top_markers.v[top_markers.v != "NDUFS5"]

### Expression level analysis - Top markers heatmap

pdf_file <- file.path(output_dir_human, "Gene_level_analysis/top_markers_heatmap/all_clusters_all_time_points_expression_level_top_markers_epilepsy.pdf")

SpatialPathways2GenesHeatmap(seurat_orth_human, clusters =  seurat_custom_cluster.v, time_points = time_points.v, genes = top_markers.v, cluster_colors = col_clusters, time_colors = time_points_col, output_file = pdf_file, separate_conditions = TRUE, cluster_hierarchisation = TRUE, time_ordered = TRUE, row_km = 2, cluster_rows = TRUE)


### Expression level analysis - Hippocampus - Top markers heatmap

pdf_file <- file.path(output_dir_human, "Gene_level_analysis/top_markers_heatmap/hippocampus_clusters_all_time_points_expression_level_top_markers_epilepsy.pdf")

SpatialPathways2GenesHeatmap(seurat_orth_human, clusters =  hippocampus_cluster.v, time_points = time_points.v, genes = top_markers.v, cluster_colors = col_clusters, time_colors = time_points_col, output_file = pdf_file, separate_conditions = TRUE, cluster_hierarchisation = TRUE, time_ordered = TRUE, row_km = 2)
...
### Top 50 negative rank score genes within each cluster and each time points #######
filtered_markers_negative_rank_score_list <- list()
for (name_markers in names(list_markers_time_point.cluster_SEvsCTRL)) {
      # Extract the current markers data frame
    markers_df <- list_markers_time_point.cluster_SEvsCTRL[[name_markers]]

    # Keep only the last 50 genes of each dataframe, i.e with the lowest rank score
    filtered_markers_negative_rank_score_list[[name_markers]] <- tail(markers_df, n = 50) 

    }

### Markers frequency across clusters and time points
marker_counts <- bind_rows(lapply(names(filtered_markers_negative_rank_score_list), function(name) {
  filtered_markers_negative_rank_score_list[[name]] %>% 
    rownames_to_column("gene")
})) %>%
  count(gene, name = "frequency") %>%
  mutate(gene = factor(gene, levels = gene[order(-frequency)]))
...
plot <- ggplot(marker_counts, aes(x = gene, y = frequency)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black", alpha = 0.7) +
  labs(
    title = "Frequency of Negative Score Marker Genes Across Clusters and Time Points",
    x = "Gene",
    y = "Frequency"
  ) +
  theme_minimal() +
...
pdf_file <- file.path(output_dir_human, "Gene_level_analysis/markers_frequency/negative_markers_frequency_cluster_time_points.pdf")
pdf(pdf_file, width = 10, height = 10)
print(plot)
dev.off()

# Most frequent markers
frequent_marker_counts <- marker_counts %>% filter(frequency >= 6)
...
pdf_file <- file.path(output_dir_human, "Gene_level_analysis/markers_frequency/top_negative_markers_frequency_cluster_time_points.pdf")
pdf(pdf_file, width = 10, height = 10)
print(plot)
dev.off()

# Select top markers
top_markers_negative.v <- frequent_marker_counts[, "gene"]

### Expression level analysis - Top markers heatmap

pdf_file <- file.path(output_dir_human, "Gene_level_analysis/top_markers_heatmap/all_clusters_all_time_points_expression_level_top_negative_markers_epilepsy.pdf")

SpatialPathways2GenesHeatmap(seurat_orth_human, clusters =  seurat_custom_cluster.v, time_points = time_points.v, genes = top_markers_negative.v, cluster_colors = col_clusters, time_colors = time_points_col, output_file = pdf_file, separate_conditions = TRUE, time_ordered = TRUE, cluster_hierarchisation = TRUE, row_km = 5)
