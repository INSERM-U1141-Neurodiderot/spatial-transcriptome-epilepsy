...
split_condition_seurat <- SplitObject(seurat_object_whole_clustering, split.by = "condition")
...
### GOBP
SCPA_human_GOPB <- scpa_subclusters(split_seurat = split_condition_seurat_orth_human, subclusters = seurat_custom_cluster.v , time_points = time_points.v, organism = "Homo sapiens", pathway_collection = "GOBP", parallel = TRUE)

## Rat
### Hallmark
SCPA_rat_HALLMARK <- scpa_subclusters(split_seurat = split_condition_seurat, subclusters = seurat_custom_cluster.v , time_points = time_points.v, organism = "Rattus norvegicus", pathway_collection = "HALLMARK", parallel = TRUE)

### Kegg
SCPA_rat_KEGG <- scpa_subclusters(split_seurat = split_condition_seurat, subclusters = seurat_custom_cluster.v , time_points = time_points.v, organism = "Rattus norvegicus", pathway_collection = "KEGG", parallel = TRUE)

### Reactome
SCPA_rat_REACTOME <- scpa_subclusters(split_seurat = split_condition_seurat, subclusters = seurat_custom_cluster.v , time_points = time_points.v, organism = "Rattus norvegicus", pathway_collection = "REACTOME", parallel = TRUE)

### GOBP
SCPA_rat_GOPB <- scpa_subclusters(split_seurat = split_condition_seurat, subclusters = seurat_custom_cluster.v , time_points = time_points.v, organism = "Rattus norvegicus", pathway_collection = "GOBP", parallel = TRUE) 
...
### GOBP
write.csv(x = SCPA_human_GOBP, file = sprintf("%s/GOBP.csv", output_dir_SCPA_human), row.names = TRUE)

## Rat
output_dir_SCPA_rat <- sprintf("%s/SCPA_analysis", output_dir_rat)
if (! dir.exists(output_dir_SCPA_rat)) {
    dir.create(output_dir_SCPA_rat, recursive=TRUE, mode="0775")
}

### HALLMARK
                     
write.csv(SCPA_rat_HALLMARK, file = sprintf("%s/HALLMARK.csv", output_dir_SCPA_rat), row.names = TRUE)

### KEGG

write.csv(SCPA_rat_KEGG, file = sprintf("%s/KEGG.csv", output_dir_SCPA_rat), row.names = TRUE)

### REACTOME
write.csv(x = SCPA_rat_REACTOME, file = sprintf("%s/REACTOME.csv", output_dir_SCPA_rat), row.names = TRUE)

### GOBP
write.csv(x = SCPA_rat_GOBP, file = sprintf("%s/GOBP.csv", output_dir_SCPA_rat), row.names = TRUE)